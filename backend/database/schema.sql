...
CREATE TABLE notas (
  ...
  media         NUMERIC(4, 1)  GENERATED ALWAYS AS (
                  CASE WHEN nota1 IS NOT NULL AND nota2 IS NOT NULL
                       THEN ROUND((nota1 + nota2) / 2.0, 1)
                       ELSE NULL
                  END
                ) STORED,
  situacao      VARCHAR(20)    GENERATED ALWAYS AS (
                  CASE
                    WHEN nota1 IS NULL OR nota2 IS NULL THEN NULL
